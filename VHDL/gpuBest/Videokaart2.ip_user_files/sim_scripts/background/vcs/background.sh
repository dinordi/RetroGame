...
#
# Script generated by Vivado on Tue Mar 05 16:22:41 +0100 2024
...
#
# Usage        : background.sh
#                background.sh [-lib_map_path] [-step] [-keep_index] [-noclean_files]*
#                background.sh [-reset_run]
#                background.sh [-reset_log]
#                background.sh [-help]
#
...
#                script (type 'background.sh -help' for more information).
#
...
# script info
echo -e "background.sh - Script generated by export_simulation (Vivado v2023.2 (64-bit)-id)\n"
...
run()
{
  ...
  if [[ ($b_step == 1) ]]; then
    case $step in
      ...
      ;;
      * )
        echo -e "ERROR: Invalid or missing step '$step' (type \"./background.sh -help\" for more information)\n"
        exit 1
      esac
  else
    init_lib
...
elaborate()
{
  vcs $vcs_elab_opts xil_defaultlib.background xil_defaultlib.glbl -o background_simv
}

# RUN_STEP: <simulate>
simulate()
{
  ./background_simv $vcs_sim_opts -do simulate.do
}
...
reset_run()
{
  files_to_remove=(ucli.key background_simv vlogan.log vhdlan.log compile.log elaborate.log simulate.log .tmp_log .vlogansetup.env .vlogansetup.args .vcs_lib_lock scirocco_command.log lib_sc.so 64 AN.DB csrc background_simv.daidir vcs_lib c.obj)
...
check_arg_value()
{
  ...
  if [[ ($1 == "-lib_map_path") && ($2 == "") ]];then
    echo -e "ERROR: Simulation library directory path not specified (type \"./background.sh -help\" for more information)\n"
    exit 1
  fi
}
...
usage()
{
  msg="Usage: background.sh [-help]\n\
Usage: background.sh [-step]\n\
Usage: background.sh [-lib_map_path]\n\
Usage: background.sh [-reset_run]\n\
Usage: background.sh [-reset_log]\n\
Usage: background.sh [-keep_index]\n\
Usage: background.sh [-noclean_files]\n\n\
